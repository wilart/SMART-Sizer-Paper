#Simulation Design 1: generates the power plot for pilot design 1
#need to run R/Simulation/design-1/5-design-1-empirical-power first and
#
...
g_power_by_n_predicted <- ggplot(pilot.power.melt, aes(x = n_grid, y = Power, color = variable, linetype = variable)) + 
  ...
  ggtitle("Pilot Design 1: Power vs. Sample Size") + guides(colour = guide_legend(override.aes = list(shape = NA)))+
  guides(colour = guide_legend(override.aes = list(shape = NA),keywidth = 2))



#pdf(file = '~/../Dropbox/SMART-R/SMART-Power-Analysis/R/Plots/SMART-Pilot-Power1.pdf', width = 6, height = 5)
